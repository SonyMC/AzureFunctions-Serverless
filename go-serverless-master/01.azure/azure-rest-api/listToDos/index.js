...
module.exports = async function (context, req) {
    ...
    const results = await todoCollection
        .find({ username: "mailsonymathew" }) //filter results using username
        .toArray()

    await connection.close()

    return {
        body: JSON.stringify(results).replace(/_id/g, "id")  // replace '_id' in output with 'id' usinging regular expression
    };
}